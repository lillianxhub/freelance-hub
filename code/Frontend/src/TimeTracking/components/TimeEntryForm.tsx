...
export default function TimeEntryForm({ value, projects, tasks, error, onChange, onSubmit, onCancel }: TimeEntryFormProps) {
  return <form onSubmit={onSubmit}>
    ...
    <div className="form-grid">
      ...
      <div className="form-field"><label htmlFor="manual-task">งาน</label><select id="manual-task" name="task_id" value={value.task_id || ''} onChange={onChange}><option value="">ไม่ระบุ</option>{tasks.filter((task) => task.project_id === value.project_id && task.status !== 'DONE').map((task) => <option key={task.id} value={task.id}>{task.name}</option>)}</select></div>
