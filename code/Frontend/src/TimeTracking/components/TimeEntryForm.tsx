...
export default function TimeEntryForm({ value, projects, tasks, error, onChange, onSubmit, onCancel }: TimeEntryFormProps) {
  return <form onSubmit={onSubmit}>
    ...
    <div className="form-grid">
      ...
      <div className="form-field"><label htmlFor="manual-task">Task</label><select id="manual-task" name="task_id" value={value.task_id || ''} onChange={onChange}><option value="">ไม่ระบุ</option>{tasks.filter((task) => task.project_id === value.project_id).map((task) => <option key={task.id} value={task.id}>{task.name}</option>)}</select></div>
      ...
      <div className="form-field"><label htmlFor="manual-rate">อัตราต่อชั่วโมง (เว้นว่างเพื่อใช้ Project rate)</label><input id="manual-rate" name="rate_snapshot" type="number" min="0" step="0.01" value={value.rate_snapshot} onChange={onChange} /></div>
