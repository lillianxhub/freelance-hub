...
const emptyTask: TaskDraft = { name: '', description: '', status: 'TODO', due_date: '' }
...
function ProjectDetailPage() {
  ...
  const effectiveRate = project.billing_type === 'FIXED_PRICE' && totalMinutes ? (project.fixed_price ?? 0) / (totalMinutes / 60) : null
  ...
  const saveTask = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await save('tasks', { ...taskForm, project_id: project.id, name: taskForm.name.trim(), sort_order: taskForm.sort_order ?? tasks.length })
    setModalOpen(false)
  }
  ...
  return (
    <div className="page-view">
      ...
      <PageHeader eyebrow="พื้นที่ทำงาน / โปรเจกต์" title={project.name} description={`${client?.company_name || client?.name} · ${project.description || 'ไม่มีรายละเอียด'}`} actions={<><StatusBadge status={project.status} /><Link className="button button-primary" to="/time-tracker">◷ เริ่มจับเวลา</Link></>} />
      ...
      <div className="summary-grid">
        ...
        <article className="metric-card accent-green"><div className="metric-top"><span>มูลค่าเกิดขึ้น</span><span className="metric-icon">฿</span></div><div className="metric-value metric-compact">{formatMoney(project.billing_type === 'FIXED_PRICE' ? project.fixed_price : billableValue, project.currency)}</div><div className="metric-foot">{project.billing_type === 'HOURLY' ? 'คำนวณจาก เวลาที่billable' : 'มูลค่า fixed price'}</div></article>
        <article className="metric-card accent-violet"><div className="metric-top"><span>งาน</span><span className="metric-icon">✓</span></div><div className="metric-value">{completed}<span className="metric-unit">/ {tasks.length}</span></div><div className="metric-foot">เสร็จแล้ว {taskProgress}%</div></article>
        <article className={`metric-card ${budgetPercent >= 100 ? 'accent-red' : 'accent-orange'}`}><div className="metric-top"><span>การใช้งบ</span><span className="metric-icon">◔</span></div><div className="metric-value">{budgetPercent}<span className="metric-unit">%</span></div><div className="metric-foot">{budgetPercent >= 100 ? 'เกินงบประมาณ' : budgetPercent >= 80 ? 'ใกล้ถึงงบประมาณ' : 'ยังอยู่ในแผน'}</div></article>
      </div>

      <div className="detail-grid">
        <div className="section-stack">
          <section className="panel">
            <div className="panel-heading"><div><h2>งาน</h2><p>สร้าง ปิดงาน แก้ไข และเรียงลำดับงานในโปรเจกต์</p></div><button className="button button-primary" type="button" onClick={() => openTask()}>＋ เพิ่ม งาน</button></div>
            <TaskList tasks={tasks} onToggle={(task) => save('tasks', { ...task, status: task.status === 'DONE' ? 'TODO' : 'DONE' })} onMove={moveTask} onEdit={openTask} onDelete={(task) => remove('tasks', task.id)} />
          </section>

          <section className="panel">
            <div className="panel-heading"><div><h2>รายการเวลาล่าสุด</h2><p>เวลาล่าสุดที่บันทึกในโปรเจกต์</p></div><Link className="text-button" to="/time-tracker">ดูทั้งหมด →</Link></div>
            <div className="table-wrap"><table className="data-table"><thead><tr><th>รายละเอียด</th><th>วันที่</th><th>ระยะเวลา</th><th>คิดค่าบริการ</th><th>มูลค่า</th></tr></thead><tbody>
        ...
        <aside className="panel">
          <div className="panel-heading"><div><h2>รายละเอียดโปรเจกต์</h2><p>ขอบเขต ราคา และกำหนดการ</p></div></div>
          <div className="detail-list">
            ...
            {effectiveRate !== null && <div className="detail-item"><span>อัตราต่อชั่วโมงโดยเฉลี่ย</span><strong>{formatMoney(effectiveRate, project.currency)}/ชม.</strong></div>}
            <div className="detail-item"><span>ความคืบหน้างาน</span><div className="progress-label"><span>{completed}/{tasks.length}</span><strong>{taskProgress}%</strong></div><div className="progress-track"><span style={{ width: `${taskProgress}%`, background: project.color }} /></div></div>
          </div>
        </aside>
      </div>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={taskForm.id ? 'แก้ไข Task' : 'เพิ่ม Task'}>
        <TaskForm value={taskForm} error={formError} onChange={setTaskForm} onSubmit={saveTask} onCancel={() => setModalOpen(false)} />
      </Modal>
    </div>
  )
}
