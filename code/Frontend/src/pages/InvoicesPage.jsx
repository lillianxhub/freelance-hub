...
function InvoicesPage() {
  ...
  const invoices = useMemo(() => (data?.invoices || [])
    ...
    .sort((a, b) => b.issue_date.localeCompare(a.issue_date)), [data, query, status])
  ...
  return (
    <div className="page-view">
      ...
      <section className="panel">
        ...
        <div className="filter-row"><label className="search-box"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="ค้นหา Invoice หรือลูกค้า..." /></label><select className="select-button" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">ทุกสถานะ</option><option value="DRAFT">ฉบับร่าง</option><option value="ISSUED">ออกแล้ว</option><option value="PAID">ชำระแล้ว</option><option value="OVERDUE">เกินกำหนด</option><option value="VOID">ยกเลิก</option></select></div>
