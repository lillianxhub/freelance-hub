...
function InvoicesPage() {
  ...
  const [sortBy, setSortBy] = useState('ISSUE_DESC')
  ...
  return (
    <div className="page-view">
      <PageHeader eyebrow="Manage / Invoices" title="Invoices" description="สร้าง ส่ง และติดตามการชำระเงินจากที่เดียว" actions={<Link className="button button-primary" to="/invoices/new">＋ สร้าง Invoice</Link>} />
      <div className="summary-grid">
        ...
        <article className="metric-card accent-green"><div className="metric-top"><span>รับชำระแล้ว</span><span className="metric-icon">✓</span></div><div className="metric-value metric-compact">{formatMoney(paid)}</div><div className="metric-foot">รายได้จาก Invoice ทั้งหมด</div></article>
        ...
        <article className="metric-card accent-blue"><div className="metric-top"><span>Invoice ทั้งหมด</span><span className="metric-icon">▤</span></div><div className="metric-value">{data.invoices.length}<span className="metric-unit">ฉบับ</span></div><div className="metric-foot">รวมทุกสถานะ</div></article>
      </div>

      <section className="panel">
        <div className="panel-heading"><div><h2>Invoice list</h2><p>ค้นหาตามเลขที่เอกสารหรือลูกค้า</p></div></div>
        <div className="filter-row"><label className="search-box"><span>⌕</span><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1) }} placeholder="ค้นหา Invoice หรือลูกค้า..." /></label><select className="select-button" value={status} onChange={(event) => { setStatus(event.target.value); setPage(1) }}><option value="ALL">ทุกสถานะ</option><option value="DRAFT">ฉบับร่าง</option><option value="ISSUED">ออกแล้ว</option><option value="PAID">ชำระแล้ว</option><option value="OVERDUE">เกินกำหนด</option><option value="VOID">ยกเลิก</option></select><select className="select-button" value={sortBy} onChange={(event) => { setSortBy(event.target.value); setPage(1) }}><option value="ISSUE_DESC">ออกล่าสุด</option><option value="DUE_ASC">ครบกำหนดใกล้สุด</option><option value="TOTAL_DESC">ยอดสูงสุด</option></select></div>
        {invoices.length === 0 ? <EmptyState icon="▤" title="ไม่พบ Invoice" description="ลองเปลี่ยนคำค้นหาหรือสร้าง Invoice ฉบับแรก" action={<Link className="button button-primary" to="/invoices/new">สร้าง Invoice</Link>} /> : (
          <div className="table-wrap"><table className="data-table"><thead><tr><th>เลขที่ Invoice</th><th>ลูกค้า</th><th>วันที่ออก</th><th>ครบกำหนด</th><th>ยอดรวม</th><th>คงเหลือ</th><th>สถานะ</th><th /></tr></thead><tbody>{visibleInvoices.map((invoice) => {
            const client = data.clients.find((item) => item.id === invoice.client_id)
