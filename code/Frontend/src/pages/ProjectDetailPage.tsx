...
import type { ResourceInput, Task, TaskStatus } from '../types/domain'
import { calculateTimeValue, formatDate, formatDuration, formatMoney } from '../utils/formatters'

type TaskForm = Omit<ResourceInput<'tasks'>, 'project_id' | 'sort_order'> & {
  project_id?: string
  sort_order?: number
}

const emptyTask: TaskForm = { name: '', description: '', status: 'TODO', due_date: '' }
...
function ProjectDetailPage() {
  ...
  const openTask = (task: Task | TaskForm = emptyTask) => {
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={taskForm.id ? 'แก้ไข Task' : 'เพิ่ม Task'}>
        <form onSubmit={saveTask}>
          ...
          <div className="form-grid">
            ...
            <div className="form-field"><label htmlFor="task-status">สถานะ</label><select id="task-status" value={taskForm.status} onChange={(event) => setTaskForm((current) => ({ ...current, status: event.target.value as TaskStatus }))}><option value="TODO">รอดำเนินการ</option><option value="IN_PROGRESS">กำลังทำ</option><option value="IN_REVIEW">ตรวจสอบ</option><option value="DONE">เสร็จแล้ว</option></select></div>
