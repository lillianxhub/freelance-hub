...
import type { WorkspaceContextValue } from '../contexts/workspaceContextValue'
import type { Invoice } from '../types/domain'
import { getErrorMessage } from '../utils/errors'
import { formatDate, formatMoney } from '../utils/formatters'

const baseDate = new Date()
const dateValue = (date: Date = baseDate): string => new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 10)
...
function InvoiceEditor({ workspace, existing = null }: InvoiceEditorProps) {
  ...
  const [selectedTimeIds, setSelectedTimeIds] = useState<string[]>(existingItems
    .map((item) => item.time_entry_id)
    .filter((id): id is string => Boolean(id)))
  const [manualItems, setManualItems] = useState<ManualInvoiceItem[]>(() => {
    const items = existingItems.filter((item) => !item.time_entry_id).map((item) => ({
      id: item.id, description: item.description, quantity: item.quantity, unit_price: item.unit_price,
    }))
    return items.length ? items : [{ id: crypto.randomUUID(), description: '', quantity: 1, unit_price: 0 }]
  })
  const [form, setForm] = useState<InvoiceEditorForm>(existing ? { issue_date: existing.issue_date, due_date: existing.due_date, tax_rate: existing.tax_rate, discount_amount: existing.discount_amount, notes: existing.notes || '' } : { issue_date: dateValue(), due_date: dateValue(dueDate), tax_rate: 7, discount_amount: 0, notes: 'ขอบคุณที่ไว้วางใจใช้บริการ' })
    ...
    try {
      ...
      for (const [index, item] of invoiceItems.entries()) {
        ...
        if (item.time_entry_id) {
          ...
          if (entry) await save('time_entries', { ...entry, invoice_id: invoiceId })
        }
      }
