...
function InvoiceDetailPage() {
  ...
  if (!invoice) return <EmptyState icon="!" title="ไม่พบใบแจ้งหนี้" description="เอกสารนี้อาจถูกลบหรือคุณไม่มีสิทธิ์เข้าถึง" action={<Link className="button button-primary" to="/invoices">กลับไป ใบแจ้งหนี้</Link>} />
  ...
  return (
    <div className="page-view invoice-detail-page">
      <Link className="back-link no-print" to="/invoices">← กลับไป ใบแจ้งหนี้</Link>
      <PageHeader eyebrow="ใบแจ้งหนี้ / รายละเอียด" title={invoice.invoice_number} description={`สร้างเมื่อ ${formatDate(invoice.created_at || invoice.issue_date)}`} actions={<><button className="button button-secondary" type="button" onClick={() => window.print()}>⤓ พิมพ์ / PDF</button>{invoice.status === 'DRAFT' && <><button className="button button-danger" type="button" onClick={deleteDraft}>ลบ ฉบับร่าง</button><Link className="button button-secondary" to={`/invoices/${invoice.id}/edit`}>แก้ไข</Link><button className="button button-primary" type="button" onClick={issueInvoices}>ออก ใบแจ้งหนี้</button></>}{['ISSUED', 'OVERDUE'].includes(displayStatus) && <><button className="button button-secondary" type="button" onClick={voidInvoices}>ยกเลิกเอกสาร</button><button className="button button-primary" type="button" onClick={openPayment}>＋ รับชำระ</button></>}</>} />
