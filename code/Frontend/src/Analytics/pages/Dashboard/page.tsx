  ...
  return (
    <div className="page-view">
      ...
      <div className="summary-grid">
        <SummaryCard label="Tracked today" icon="◷" value={(todayMinutes / 60).toFixed(1)} unit="ชม." accent="blue" foot={<><span className={trend(todayMinutes, yesterdayMinutes) >= 0 ? "trend-positive" : "negative-money"}>{trend(todayMinutes, yesterdayMinutes) >= 0 ? "↗" : "↘"} {Math.abs(trend(todayMinutes, yesterdayMinutes)).toFixed(0)}%</span> เทียบเมื่อวาน</>} />
        <SummaryCard label="This week" icon="▦" value={(weekMinutes / 60).toFixed(1)} unit="ชม." accent="green" foot={<><span className={trend(weekMinutes, previousWeekMinutes) >= 0 ? "trend-positive" : "negative-money"}>{trend(weekMinutes, previousWeekMinutes) >= 0 ? "↗" : "↘"} {Math.abs(trend(weekMinutes, previousWeekMinutes)).toFixed(0)}%</span> เทียบสัปดาห์ก่อน</>} />
      ...
      <div className="dashboard-grid">
        <section className="panel chart-panel"><div className="panel-heading"><div><h2>Activity overview</h2><p>ชั่วโมงที่บันทึกใน 7 วันล่าสุด</p></div><Link className="mini-button text-link" to="/reports">ดูทั้งหมด</Link></div><ProductivityChart data={chartData} /></section>
      ...
      <section className="panel recent-time-panel"><div className="panel-heading"><div><h2>Recent time entries</h2><p>กิจกรรมล่าสุดใน Workspace</p></div><Link className="mini-button text-link" to="/time-tracker">ดู Time tracker</Link></div><RecentActivity entries={recentEntries} projects={data.projects} /></section>
    </div>
  )
}
