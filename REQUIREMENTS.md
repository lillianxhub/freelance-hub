...
## 22. แผนแบ่งงานทีม 5 คนและการทำงานแบบ Agile/Scrum (MVP เท่านั้น)

### 22.1 เป้าหมายและกำหนดส่ง

โครงการมีเวลาพัฒนา 3 สัปดาห์ และนำเสนอวันที่ **10 ตุลาคม 2026** โดย MVP ต้องมีเฉพาะ 5 กลุ่มฟีเจอร์ต่อไปนี้:

1. Authentication
2. Client Management
3. Project และ Task Management
4. Time Tracking
5. Dashboard, Analytics และ Productivity Insights

Income, Expense, Invoice และ Payment ไม่อยู่ใน MVP และไม่ใช้เป็นเกณฑ์ตรวจรับรอบนี้

### 22.2 บทบาทและเจ้าของงาน

ให้แทน `สมาชิก A-E` ด้วยชื่อจริงก่อนเริ่ม Sprint และให้แต่ละคนเป็น owner ของโค้ดอย่างน้อยหนึ่งส่วน ทุกคนต้องเขียนโค้ดจริงและรับผิดชอบเอกสาร/การทดสอบประกอบงานของตนเอง

| สมาชิก | Feature owner และงานโค้ดที่ต้องส่ง | งานเอกสาร/หลักฐาน | ผู้ช่วยสำรอง |
|---|---|---|---|
| สมาชิก A: TODO | Authentication: User/UserProfile, registration/login, security, ownership guard และ API tests | Use Case, security section, `solid-analysis.md` ส่วน Dependency Inversion | สมาชิก E |
| สมาชิก B: TODO | Client Management: Client entity/repository/service/DTO/controller, CRUD, pagination/sorting และ tests | Use Case/sequence ของ Client, data dictionary ส่วน Client | สมาชิก A |
| สมาชิก C: TODO | Project/Task: entity relation, status transition, CRUD, validation และ project state tests | Domain/Class/State diagram ของ Project, `design-patterns.md` ส่วน State | สมาชิก B |
| สมาชิก D: TODO | Time Tracking: timer start/stop, manual entry, duration calculation, concurrency guard และ tests | Sequence/Activity ของ Time Tracking, `solid-analysis.md` ส่วน SRP/OCP/LSP | สมาชิก C |
| สมาชิก E: TODO | Dashboard/Analytics: summary queries, utilization/productivity metrics, Thymeleaf dashboard และ integration tests | Component/Deployment diagram, `solid-analysis.md` ส่วน ISP, README และ demo script | สมาชิก D |

งานที่ทุกคนต้องร่วมกันทำ:

- ใช้ Layered Architecture, DTO + Mapper, validation และ constructor injection ใน feature ของตนเอง
- เพิ่ม unit/integration/security test ให้ feature ของตนเอง และช่วย review ของเพื่อนอย่างน้อย 2 Pull Requests
- เขียน commit ด้วยบัญชี GitHub ของตนเองอย่างน้อย **15 meaningful commits** กระจายตลอด 3 สัปดาห์
- อธิบายโค้ดของตนเองได้ในการนำเสนอ และแนบหลักฐานใน Git history, Pull Request และ test report
- ก่อน merge ต้องมี reviewer อย่างน้อย 1 คน และห้ามคนเดียว approve งานของตนเอง

### 22.3 แผน Sprint 3 สัปดาห์

| ช่วงเวลา | Sprint Goal | งานที่ต้องเสร็จ | ผลส่งมอบ/เกณฑ์ผ่าน |
|---|---|---|---|
| สัปดาห์ที่ 1: 19–25 ก.ย. | Foundation และ Authentication | ยืนยัน schema MVP 6 ตาราง, migration, package layers, base error response, auth, ownership, CI build/test | สมัคร/เข้าสู่ระบบได้, app ต่อ PostgreSQL ได้, ทุกคนมี branch และเริ่ม commit แล้ว |
| Progress 19 ก.ย. | Kickoff และตรวจ Foundation | ยืนยัน backlog, schema, architecture, branch และแบ่ง task | ทุกคนมี task/owner/branch และไม่มี blocker ที่ไม่รับผิดชอบ |
| สัปดาห์ที่ 2: 26 ก.ย.–2 ต.ค. | Core Work Management | Client CRUD, Project/Task CRUD, status, validation, pagination/sorting, Time Tracking เริ่ม/หยุด/manual | Flow สร้าง Client → Project → Task → Time Entry ทำงานครบ |
| Progress 26 ก.ย. | ตรวจ Sprint 2 | Demo core flow, ตรวจ API status code, test coverage และ merge เข้า `develop` | MVP core workflow ผ่าน integration test |
| สัปดาห์ที่ 3: 3–9 ต.ค. | Analytics, UI และเอกสาร | Dashboard, productivity metrics, Thymeleaf screens, Swagger, diagrams, SOLID/pattern docs, Docker | MVP demo end-to-end และเอกสารรายวิชาครบ |
| Progress 3 ต.ค. | ตรวจ Release Candidate | ทดสอบ regression, security isolation, Docker deploy และแก้ defect | Release candidate พร้อม freeze feature |
| 4–9 ต.ค. | Stabilization และ Presentation | bug fix เท่านั้น, deploy public URL, เตรียม slide/demo script, ซ้อมนำเสนอ, ตรวจ checklist | `main` มี version ส่งมอบ, URL ใช้งานได้, ทุกคนพร้อมอธิบายงาน |
| 10 ต.ค. | Presentation | นำเสนอ MVP และตอบคำถามการออกแบบ | ส่งมอบตาม rubric ของรายวิชา |

### 22.4 Scrum Ceremony และการนัด Progress

- **Daily async stand-up:** สมาชิกทุกคนรายงานในทีมทุกวันก่อน 20:00 น. ด้วยรูปแบบ `เมื่อวาน / วันนี้ / blocker`
- **Weekly Progress ทุกวันเสาร์:** 19 และ 26 กันยายน, 3 ตุลาคม เวลา `TODO: เวลา` ผ่าน `TODO: ช่องทาง` โดย demo ต้องใช้ branch ที่ merge แล้วใน `develop`
- **Sprint Planning:** วันอาทิตย์หลัง Progress กำหนด task และ acceptance criteria ของสัปดาห์ถัดไป
- **Backlog Refinement:** กลางสัปดาห์ 30 นาที เพื่อแตก task ที่ยังใหญ่เกินไปและจัดลำดับ blocker
- **Sprint Review:** รวมอยู่ใน Progress วันเสาร์ โดยแสดงของที่รันได้จริง
- **Retrospective:** หลัง Progress ทุกวันเสาร์ บันทึก `ทำได้ดี / ต้องปรับ / action owner` ใน issue หรือเอกสารทีม
- **Release freeze:** หลัง Progress วันที่ 3 ตุลาคม ห้ามเพิ่ม feature ใหม่ เหลือเฉพาะ bug/security/deployment fix

### 22.5 Definition of Done ต่อ Feature

Feature จะนับว่าเสร็จเมื่อครบทุกข้อ:

1. มี Controller → Service → Repository ตาม Layered Architecture และไม่ข้ามชั้น
2. มี Entity/DTO/Mapper/validation/error handling ครบตาม use case
3. มี unit test และ integration test ที่ครอบคลุม success, validation, not found และ ownership failure
4. API ใช้ HTTP method/status code ถูกต้อง และมี Swagger/OpenAPI description
5. มี Pull Request, reviewer, meaningful commit และไม่มี merge conflict ใน `develop`
6. อัปเดต diagram/data dictionary/README ที่เกี่ยวข้องแล้ว
7. Demo ได้จริงจาก Docker Compose และไม่พึ่งข้อมูลหรือขั้นตอนนอกระบบ

### 22.6 แผนรักษาคะแนนเต็มรายบุคคล

| เงื่อนไขใบงาน | หลักฐานที่ต้องมีต่อสมาชิก |
|---|---|
| งานโค้ดจริง | PR ที่มีไฟล์ production code ของ feature owner และ test ของตนเอง |
| Commit อย่างน้อย 15 ครั้ง | Git log ของ branch ส่วนตัว กระจายอย่างน้อย 3 สัปดาห์ |
| Branch ถูกต้อง | branch รูปแบบ `ชื่อ_รหัสนักศึกษา_section` และ push ด้วยบัญชีเจ้าของ |
| Code review | เป็น reviewer ของเพื่อนอย่างน้อย 2 PR และมี comment ที่มีสาระ |
| SOLID/Patterns | แถวชื่อและเลขบรรทัดใน `doc/solid-analysis.md` หรือ `doc/design-patterns.md` |
| อธิบายงานได้ | สไลด์ระบุ feature owner และซ้อมตอบ design/API/test/deployment |
| ไม่ outsource | ทุก commit ใช้บัญชีตนเอง ห้ามฝาก commit/push และห้ามคัดลอกโค้ดกลุ่มอื่น |

### 22.7 MVP Release Checklist

- [ ] Authentication และ ownership isolation ทำงานจริง
- [ ] Client CRUD พร้อม validation และ pagination/sorting อย่างน้อย 1 endpoint
- [ ] Project/Task CRUD และ Project state transition ทำงานจริง
- [ ] Timer start/stop, manual time entry, duration และ duplicate-timer guard ทำงานจริง
- [ ] Dashboard แสดง tracked hours, utilization, project progress และ productivity insights
- [ ] มี PostgreSQL migration, FK, index, One-to-One และ One-to-Many ครบ
- [ ] มี REST API, Thymeleaf frontend, Swagger UI และ Global Exception Handler
- [ ] มี unit/integration/security tests และ test report
- [ ] มี Dockerfile, docker-compose.yml, public deployment URL และ health check
- [ ] มี diagrams, SOLID analysis, design-patterns, data dictionary, slides และ README ครบ
- [ ] สมาชิกทั้ง 5 คนมีงานโค้ด, 15 commits, PR/review และหลักฐานใน GitHub ครบ
